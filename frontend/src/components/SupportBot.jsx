      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="fixed bottom-8 right-8 w-16 h-16 rounded-3xl shadow-[0_15px_30px_-5px_rgba(99,102,241,0.3)] z-[100] bg-indigo-600 flex items-center justify-center group overflow-hidden"
          >
            <div className="absolute inset-0 bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-500 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <Bot size={28} className="text-white relative z-10 group-hover:rotate-12 transition-transform duration-300" />

            {/* Pulsing indicator */}
            <span className="absolute top-4 right-4 w-3 h-3 bg-green-400 rounded-full border-2 border-indigo-600 z-20 animate-pulse" />
          </motion.button>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-8 right-8 w-96 max-w-[calc(100vw-3rem)] h-[600px] bg-slate-50/95 backdrop-blur-xl rounded-[2.5rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.25)] border border-white flex flex-col overflow-hidden z-[1000] ring-1 ring-black/5"
          >
            {/* Premium Glass Header */}
            <div className="relative p-7 bg-white/40 border-b border-black/5">
            ...
            <div className="px-7 py-2 bg-indigo-50/50 flex items-center gap-2">
              <ShieldCheck size={12} className="text-indigo-400" />
              <span className="text-[10px] font-bold text-indigo-400/80 uppercase tracking-widest">
                Ask about admissions, events, or school info
              </span>
            </div>
